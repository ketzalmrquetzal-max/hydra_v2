...
class OrderRequest(BaseModel):
    ...
    leverage: int = Field(default=1, ge=1, le=10, description="Apalancamiento solicitado")
    stop_loss_pct: float = Field(..., gt=0, lt=0.5, description="Stop Loss en % (ej: 0.02 = 2%)")
    take_profit_pct: float | None = Field(None, gt=0, description="Take Profit en % (opcional)")
    
    # Metadatos para el Black Box Recorder
    strategy_name: str = Field(default="BALAM_V2", description="Nombre de la estrategia")
...
class OrderApproval(BaseModel):
    ...
    signature: str | None = Field(
        None, 
        description="Firma criptográfica SHA-256 (solo si approved=True)"
    )
    ...
    rejection_details: str | None = Field(
        None,
        description="Detalles adicionales del rechazo"
    )
    
    # Metadatos
    timestamp: datetime = Field(default_factory=datetime.utcnow)
    current_daily_loss_pct: float = Field(..., description="Pérdida acumulada hoy (%)")
    remaining_risk_budget: float = Field(..., description="Cuánto más se puede arriesgar hoy (%)")
...
class RiskLimits(BaseModel):
    ...
    max_risk_per_trade_pct: float = Field(
        ...
        ge=0.001,
        le=0.05,
        description="Ley 2: Regla del 1%"
    )
    heartbeat_timeout_seconds: int = Field(
        default=60,
        ge=10,
        le=300,
        description="Ley 3: Timeout del Heartbeat"
    )
